...
def base_morgan(N,T,L,m,length,n,seed):
    #Set the follwing model as the default model.
    demography = msprime.Demography()
    demography.add_population(name="A", initial_size=N[0])
    demography.add_population(name="ADMIX", initial_size=N[1])
    demography.add_population(name="B1", initial_size=N[2])
    demography.add_population(name="B2", initial_size=N[3])
    demography.add_population(name ='B_ANC', initial_size = N[4])
    demography.add_population(name = 'ANC', initial_size = N[5])
    demography.add_admixture(time=T[0], derived='ADMIX', ancestral=["A","B1"],proportions = [0.25,0.75])
    demography.add_population_split(time=T[1], derived=["B1", "B2"], ancestral="B_ANC")
    demography.add_population_split(time = T[2], derived = ['A','B_ANC'], ancestral = 'ANC')
...
T1_list = [10,20,50]
T2_list = [60,100,200,250]
T3_list = [260,400,1000]
seeds = np.arange(300,1600,100)
# for seed in seeds:
...
for seed in seeds:
    all_data = pd.DataFrame({'index':[],'number':[],'fraction':[],'u':[],'v':[],'group':[],'true_N':[],'true_T1':[],'true_T2':[], 'true_T3':[]})
    for t1 in T1_list:
        for t2 in T2_list:
            for t3 in T3_list:
                for population_size in N_list:
                    N = [population_size]*6
                    T = [t1] + [t2] + [t3]
                    output = base_morgan(N,T,L,2,1e6,5,seed)
                    all_data = pd.concat([all_data,pd.DataFrame(output)],ignore_index=True)
                    print(f'case_{t1}_{t2}_{t3}_{population_size}_{seed} done')
    out = all_data.to_dict(orient = 'list')
    with open(f'base_{seed}.json','w') as json_file:
        json.dump(out,json_file)