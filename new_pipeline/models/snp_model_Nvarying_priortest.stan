// Copy of snp_model_Nvarying.stan with Ne gamma prior parameters passed in as data,
// so we can test how the inferred Ne responds to the prior choice without recompiling.
...
data {
    ...
    // Ne prior parameters (the whole point of this variant)
    real<lower=0> ne_prior_shape;
    real<lower=0> ne_prior_rate;
}
...
model {
    ...
    Ne ~ gamma(ne_prior_shape, ne_prior_rate);
