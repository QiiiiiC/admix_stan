...
transformed parameters {
    ...
    for (p in 1:n_leaf_pairs) {
        ...
        // Two separate W matrices: IBD needs survival, SNP does not
                ...
                if (w_diag > 1e-20) {
                    for (b in 1:n_bins) {
                        ...
                        ibd_number[b][li, lj] += w_diag *
                            int_p_LL(Ne_epoch[e], t_start, t_end,
                                     bin_length[b, 1], bin_length[b, 2]);
                    }
                    W_ibd[a, a] = w_diag * exp(-duration / Ne_epoch[e]);
                }
            }
        }
        ...
        for (e in 1:n_events) {
            ...
            for (a in 1:n_nodes) {
                ...
                if (w_diag > 1e-20) {
                    snp_mean[li, lj] += w_diag * duration / Ne_epoch[e];
                }
            }
        }
