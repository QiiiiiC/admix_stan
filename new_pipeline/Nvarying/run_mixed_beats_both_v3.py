"""
Mixed-beats-both experiment v3:
  Designed so each single-data model fails on exactly ONE of the two
  admixture events while Mixed catches both. Exploits the asymmetry:
    - IBD signal scales linearly in admixture fraction f, dies for old
      events when bins start at 0.5 cM (cuts off segments < ~1/(2T) M).
    - SNP signal scales as f^2 (covariance), age-insensitive.

True topology (T_true): 5 populations (a, b, c, d, e) with TWO admixture events:
  - RECENT admixture on b (t=20): 5% from a-side (bP1), 95% from c-side (bP2)
      -> IBD: ~5% of long (~2.5 cM) segments cross the admix path. Detectable.
      -> SNP: f^2 = 0.0025 in covariance. Below noise floor.
  - OLD admixture on d (t=700): 30% from c-side (dP1), 70% from e-side (dP2)
      -> IBD: segments ~0.07 cM, well below 0.5 cM bin floor. Invisible.
      -> SNP: f^2 = 0.09. Detectable in covariance.

  Going backward in time:
    t=20:   b splits -> bP1, bP2               (recent admixture)
    t=50:   a + bP1 -> ab
    t=100:  c + bP2 -> cb
    t=700:  d splits -> dP1, dP2               (old admixture)
    t=900:  cb + dP1 -> cbd
    t=1100: ab + cbd -> left
    t=1300: e + dP2 -> right
    t=1500: left + right -> root

  Ne: b=15000 (large -> minimal post-admix drift hides recent signal from SNP),
      d=5000  (small -> sharp covariance signal of old admix),
      rest=10000 (haploid)
...
Expected pattern:
  IBD-only: dELBO(true-alt1) >> 0 (sees recent), dELBO(true-alt2) ~ 0 (misses old)
  SNP-only: dELBO(true-alt1) ~ 0  (misses recent), dELBO(true-alt2) > 0 (sees old)
...
cm_values = [50, 100, 150, 200, 250, 300, 400, 500, 750, 1000]

# IBD bins from 0.5 cM. Crucial: omitting the 0.2-0.5 cM tail prevents
# IBD from resolving the OLD (t=700) admix, whose mean segment length
# is ~1/(2 * 700 * 0.01) cM ~ 0.07 cM.
bins = [
...
NE_B = 15000   # large -> minimal post-admix drift on b
NE_D = 5000    # small  -> sharper covariance signal for old admix
NE_REST = 10000

# Admixture fractions: fraction_parent_1 = fraction to FIRST named parent.
# RECENT (b): bP1 = a-side, bP2 = c-side. We want a tiny minor contribution
#   from a-side -> fraction_parent_1 = 0.05 (5% from a-side).
# OLD (d): dP1 = c-side, dP2 = e-side. Moderate minor contribution from
#   c-side -> fraction_parent_1 = 0.30 (30% from c-side).
ADMIX_FRAC_RECENT = 0.05
ADMIX_FRAC_OLD = 0.30
...
dem_sim.set_admixture_parameters('b', time=20,
                                 ...
                                 parent_1_name='bP1')
dem_sim.set_admixture_parameters('d', time=700,
...
dem_sim.set_merge_time('cbd', 900)
dem_sim.set_merge_time('left', 1100)
dem_sim.set_merge_time('right', 1300)
...
fig.suptitle(
    "Mixed beats both v3: T_true (2 admix) vs T_alt1 (old only) vs T_alt2 (recent only)\n"
    f"Ne: b={NE_B}, d={NE_D}, rest={NE_REST}  |  "
    f"recent: t=20, frac_a-side={ADMIX_FRAC_RECENT} (5%)  |  "
    f"old: t=700, frac_c-side={ADMIX_FRAC_OLD} (30%)  |  "
    f"bins from {bins[0][0]} cM",
    fontsize=12, fontweight="bold"
)
fig.tight_layout(rect=[0, 0, 1, 0.94])
fig.savefig("mixed_beats_both_v3.png", dpi=200, bbox_inches="tight")
plt.show()
print("Saved: mixed_beats_both_v3.png")
...
fig2.suptitle(
    "Admixture parameter estimation (v3)\n"
    f"True: b admix at t=20 (frac={ADMIX_FRAC_RECENT}), "
    f"d admix at t=700 (frac={ADMIX_FRAC_OLD})  |  "
    f"Ne: b={NE_B}, d={NE_D}, rest={NE_REST}",
    fontsize=13, fontweight="bold"
)
fig2.tight_layout(rect=[0, 0, 1, 0.94])
fig2.savefig("mixed_beats_both_v3_params.png", dpi=200, bbox_inches="tight")
plt.show()
print("Saved: mixed_beats_both_v3_params.png")
...
plot_relative_error_boxplot(
    ...
    outpath="mixed_beats_both_v3_relative_error.png",
...
# ====================================================================
print("\n" + "=" * 110)
print("MIXED BEATS BOTH v3 — SUMMARY")
print("=" * 110)
