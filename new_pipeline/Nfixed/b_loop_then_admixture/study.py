"""Recent loop in b followed backwards by admixture: data and candidate graphs."""
...
def simulation_demography(c, scenario):
    ...
    admixed = "anc_b" if scenario == "b_loop" else "b"
    d.add_admixture_event(admixed, "b1", "b2")
    d.add_merge_event("a", "b1", "left")
    d.add_merge_event("b2", "c", "right")
    d.add_merge_event("left", "right", "root")
    for name in d.nodes:
        d.set_node_ne(name, c["haploid_ne"] / 2)
    if scenario == "b_loop":
        d.set_admixture_parameters("b", t["loop_open"], c["fractions"]["loop"], "loop1")
        d.set_merge_time("anc_b", t["loop_close"])
    d.set_admixture_parameters(admixed, t["b_split"], c["fractions"]["b"], "b1")
    ...
    for k, ev in enumerate(candidate["events"]):
        if ev["type"] == "ADMIXTURE":
            ...
            mapping.append(dict(name="loop_major_fraction" if is_loop else "b_fraction",
                                ...
                                fold=is_loop))
            admixture_index += 1
            for p in ev["parents"]: clades[p] = {p}
        else:
            members = set.union(*(clades[p] for p in ev["children"]))
