...
`simulate --block-slice k/n` and `fit --candidate-slice k/n` fits every n-th candidate starting at k, so several
workers can share one run directory: each result lives in its own folder and is
skipped once written.  Compile both models first (`compile`) so concurrent
workers never race on the executable.
"""
...
from study import (HERE, PIPELINE, REAL_DATA, POPS, SCENARIOS, SOURCES, VARIANTS, UPPER,
                   pair_counts as data_pair_counts, load_config, edges, candidates, selections,
                   msprime_demography, quiet, true_ibd, hapibd, snp_summaries, aggregate,
                   stan_data, save_json, digest, event_parameters, branch_map, reference,
                   residual_tilt)
        ...
        for pool in range(c["pools"]):
            ...
            for block in range(c["blocks"]):
                if a.block_limit is not None and block >= a.block_limit:
                    break
                if a.block_slice and block % a.block_slice[1] != a.block_slice[0]:
                    continue
...
def summaries(sv, weights, obs, candidate, c, scenario):
    ...
    result["residual_tilt"] = residual_tilt(pearson, c)
...
def fit_one(model, data, obs, c, variant, candidate, folder, scenario):
    import arviz as az
    ...
    base.update(summaries(sv, weights, obs, candidate, c, scenario))
    return base
...
def fit_stage(c, a, calibration=False):
    ...
    if a.candidate_slice:
        k, n = (int(v) for v in a.candidate_slice.split("/"))
        chosen_candidates = [x for i, x in enumerate(chosen_candidates) if i % n == k]
                ...
                for rep, selected in enumerate(selections(c, pool)):
                    ...
                    for source in a.sources:
                        ...
                        for cand in chosen_candidates:
                            ...
                            if calibration:
                                result = mcmc_one(model, data, obs, c, variant, cand, folder, a, scenario)
                            else:
                                result = fit_one(model, data, obs, c, variant, cand, folder, scenario)
...
def mcmc_one(model, data, obs, c, variant, candidate, folder, a, scenario):
...
def main():
    ...
    a.block_slice = tuple(int(v) for v in a.block_slice.split("/")) if a.block_slice else None
    ...
    if a.stage == "plan":
        ...
        from study import mrca_library
        mrca_library()
        for variant in a.variants:
            model = normalized_model(variant, a.out)
