"""
FastSMC version of the Nfixed admix-through-b + ancient-admix experiment.

Mirrors run_admix_thru_b_plus_ancient_hapibd.py exactly, except IBD segments
...
SAMPLES_PER_POP = {'a': 20, 'b': 20, 'c': 20, 'd': 20}# Haploid sample count per population (diploid samples x ploidy 2)
...
FASTSMC_CSFS_SAMPLES  = sum(N_HAPLOID_PER_POP.values())   # 4 pops x 15 dip x 2 = 120
...
# ====================================================================
print("=" * 60)
print("FastSMC estimation pipeline -- Nfixed admix-through-b + ancient")
...
_topo_ax.set_title('admix_thru_b_plus_ancient (true topology)')
_topo_fig.savefig(os.path.join(_HERE, 'topology_admix_thru_b_plus_ancient.png'),
                  dpi=200, bbox_inches='tight')
plt.close(_topo_fig)
print(f"[topology] saved {os.path.join(_HERE, 'topology_admix_thru_b_plus_ancient.png')}")
...
plot_delbo_boxplot(
    ...
    outpath=os.path.join(_HERE, "admix_thru_b_plus_ancient_dELBO_Nfixed_fastsmc.png"),
    suptitle=(f"FastSMC Nfixed (uniform Ne={NE_UNIFORM}): dELBO comparison  |  "
              f"recent t={T_LOOP_SPLIT}->{T_APB_MERGE}->{T_LOOP_CLOSE}, "
              f"alpha={ALPHA_LOOP}  |  ancient t={T_OLD}, f={F_OLD}  |  "
...
plot_param_comparison(
    ...
    outpath=os.path.join(_HERE, "admix_thru_b_plus_ancient_params_Nfixed_fastsmc.png"),
...
plot_relative_error_boxplot(
    ...
    outpath=os.path.join(_HERE, "admix_thru_b_plus_ancient_relative_error_Nfixed_fastsmc.png"),
