...
parameters {
    // times[1] is the excess beyond generation 10.  This makes the two fixed
    // recent epochs well-defined for every leaf while allowing the first
    // demographic event anywhere after generation 10.
    ...
    matrix[n_leaves, 2] Ne_recent_raw; // increments for 0-5 and 5-10 generations
...
transformed parameters {
    ...
    // Fixed recent grid.  Index 1 is 0-5 generations and index 2 is 5-10.
    // Build from the post-10 leaf branch toward the present, using exactly the
    // same time-scaled random-walk prior as the subsequent branch process.
    matrix[n_leaves, 2] log_Ne_recent;
    matrix<lower=0>[n_leaves, 2] Ne_recent;
    for (a in 1:n_leaves) {
        log_Ne_recent[a, 2] = log_Ne[a]
            + tau * sqrt(5.0 / t_ref) * Ne_recent_raw[a, 2];
        log_Ne_recent[a, 1] = log_Ne_recent[a, 2]
            + tau * sqrt(5.0 / t_ref) * Ne_recent_raw[a, 1];
    }
    Ne_recent = exp(log_Ne_recent);
    ...
    for (p in 1:n_leaf_pairs) {
                ...
                if (w_diag > 1e-20) {
                    for (b in 1:n_bins) {
                        ibd_fraction[b][li, lj] += w_diag *
                            int_p_L(Ne_recent[a, g], t_start, t_end,
                                    bin_length[b, 1], bin_length[b, 2]);
                        ibd_number[b][li, lj] += w_diag *
                            int_p_LL(Ne_recent[a, g], t_start, t_end,
                                     bin_length[b, 1], bin_length[b, 2]);
                    }
                    W_ibd[a, a] = w_diag * exp(-5.0 / Ne_recent[a, g]);
                }
            }
        }
        ...
        for (g in 1:2) {
            for (a in 1:n_leaves) {
                ...
                if (w_diag > 1e-20)
                    snp_mean[li, lj] += w_diag * 5.0 / Ne_recent[a, g];
            }
        }
...
model {
    ...
    Ne_raw    ~ std_normal();
    to_vector(Ne_recent_raw) ~ std_normal();
